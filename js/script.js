...
// ===============================
// WhatsApp order messages (exact choice, current language)
// ===============================
const WA_NUMBER = "201551371870";
const WA_MSG = {
  order: {
    ar: (c) => "أهلاً Everyfar، عايز أطلب: " + c,
    en: (c) => "Hello Everyfar, I want to order: " + c
  },
  inquire: {
    ar: (c) => "أهلاً Everyfar، عايز أستفسر عن: " + c,
    en: (c) => "Hello Everyfar, I want to ask about: " + c
  },
  general: {
    ar: "أهلاً Everyfar، عايز أستفسر عن خدماتكم.",
    en: "Hello Everyfar, I want to ask about your services."
  }
};
function waCurrentLang(){ return document.documentElement.lang === "en" ? "en" : "ar"; }
function waBuildLink(a){
  const lang = waCurrentLang();
  const choice = a.getAttribute(lang === "ar" ? "data-order-ar" : "data-order-en");
  if (choice) return "https://wa.me/" + WA_NUMBER + "?text=" + encodeURIComponent(WA_MSG.order[lang](choice));
  const ask = a.getAttribute(lang === "ar" ? "data-inquire-ar" : "data-inquire-en");
  const msg = ask ? WA_MSG.inquire[lang](ask) : WA_MSG.general[lang];
  return "https://wa.me/" + WA_NUMBER + "?text=" + encodeURIComponent(msg);
}
function waRefreshLinks(){
  document.querySelectorAll('a[href*="wa.me"]').forEach((a)=>{
    a.href = waBuildLink(a);
    a.target = "_blank";
    a.rel = "noopener";
  });
}
document.addEventListener("DOMContentLoaded", waRefreshLinks);
document.addEventListener("click", (e)=>{
  if (!e.target.closest) return;
  if (e.target.closest("#lang")) { setTimeout(waRefreshLinks, 60); return; }
  const a = e.target.closest('a[href*="wa.me"]');
  if (a) a.href = waBuildLink(a);
});
